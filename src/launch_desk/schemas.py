...
class LaunchDeskRequest(BaseModel):
    product_brief: str = Field(
        ..., min_length=10, description="Rough product or feature launch brief."
    )
    ...
    constraints: list[str] = Field(
        default_factory=list, description="Hard limits, approvals, or launch constraints."
    )
    assets: list[str] = Field(
        default_factory=list, description="Existing launch assets and collateral."
    )
...
class LaunchRiskItem(BaseModel):
    ...
    severity: Literal["low", "medium", "high", "critical"] = Field(
        ..., description="Impact if the risk happens."
    )
    likelihood: Literal["low", "medium", "high"] = Field(
        ..., description="Likelihood before mitigation."
    )
...
class OwnerChecklistItem(BaseModel):
    ...
    checklist: list[str] = Field(
        default_factory=list, description="Concrete checklist items for this owner."
    )
...
class FollowUpQuestion(BaseModel):
    ...
    blocking: bool = Field(
        default=True, description="Whether the missing answer blocks launch planning."
    )
...
class ReadinessRubricItem(BaseModel):
    ...
    status: Literal["ready", "partial", "blocked"] = Field(
        ..., description="Status for this dimension."
    )
...
class ReadinessSummary(BaseModel):
    ...
    verdict: Literal["go", "go_with_risks", "hold"] = Field(
        ..., description="Launch decision recommendation."
    )
    top_gaps: list[str] = Field(default_factory=list, description="Most important remaining gaps.")
    rubric: list[ReadinessRubricItem] = Field(
        default_factory=list, description="Readiness rubric details."
    )
...
class LaunchDeskReport(BaseModel):
    ...
    assumptions: list[str] = Field(
        default_factory=list, description="Assumptions made from incomplete inputs."
    )
    follow_up_questions: list[FollowUpQuestion] = Field(
        default_factory=list, description="Questions for missing launch details."
    )
    prioritized_plan: list[LaunchPlanStep] = Field(
        default_factory=list, description="Prioritized actionable launch plan."
    )
    risk_register: list[LaunchRiskItem] = Field(
        default_factory=list, description="Launch risk register."
    )
    owner_checklist: list[OwnerChecklistItem] = Field(
        default_factory=list, description="Owner-by-owner launch checklist."
    )
    launch_copy_suggestions: list[CopySuggestion] = Field(
        default_factory=list, description="Channel-specific launch copy drafts."
    )
