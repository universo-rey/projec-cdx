...
$caseRoot = Join-Path $testRoot ("feedback-" + [guid]::NewGuid().ToString())
...
& (Join-Path $root "tools\ceo-alert-engine.ps1") -EventStoreRoot $store -TraceRoot $traceRoot | Out-Null
$result = (& (Join-Path $root "tools\ceo-policy-feedback.ps1") -EventStoreRoot $store -TraceRoot $traceRoot) | ConvertFrom-Json
if (-not $result.recommendations_generated) { throw "RECOMMENDATIONS_NOT_GENERATED" }
if (-not $result.suggestion_only) { throw "RECOMMENDATIONS_NOT_SUGGESTION_ONLY" }

[ordered]@{
    status = "PASS"
    recommendation_count = [int]$result.recommendation_count
    suggestion_only = [bool]$result.suggestion_only
