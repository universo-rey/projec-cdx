...
$store = Join-Path (Join-Path $testRoot ("policy-" + [guid]::NewGuid().ToString())) "event-store"
$event = [ordered]@{
...
$raw = & powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $root "tools\ceo-policy-engine.ps1") -EventFile $eventPath -EventStoreRoot $store
